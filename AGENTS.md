...
- `open_law_lens/statutes.py` must fail closed on unsupported/conflicting qualified citations and LegInfo pages without a matching statutory body; only genuinely bare sections default to WIC. Keep parsing/link aliases consistent. Validation failures must precede authority-cache writes; do not delete old cached results as part of code maintenance.
...
- `open_law_lens/library.py` owns the durable SQLite case library, display-text extraction, reporter page-marker offsets, and official-copy outcome cache. `CaseLibrary.ensure()` must stay read-only whenever the stored schema version and every one-time migration marker are current, so concurrent CLI process startups cannot collide on the SQLite write lock; any new migration must bump `SCHEMA_VERSION` or introduce a fresh marker key rather than relying on `ensure()` re-running unconditionally.
