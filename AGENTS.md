...
- `open_law_lens/app.py` is the GTK/Libadwaita desktop app, including the citation lookup UI, cached-case browser, opinion reader, settings dialog, and embedded Codex terminal workflow.
...
- `scripts/open-law-lens-codex-agent-vte.sh` launches Codex from the embedded terminal. Preserve its temporary-workspace and cache-directory behavior unless the task explicitly changes agent launch semantics.
